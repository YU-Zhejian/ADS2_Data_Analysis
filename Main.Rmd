...
ggplot(sub_use_alcohol, aes(x = year, y = val, color = age, shape = sex)) +
...
ggplot(sub_use_Opioid, aes(x = year, y = val, color = age)) +
        geom_point(show.legend = TRUE) +
        geom_line(show.legend = TRUE) +
...
```{r FAILED IDEA}
for (col_name in names(sub_use)) {
  if (class(sub_use[[col_name]]) == "factor") {
    sub_use[[col_name]] <- as.character(sub_use[[col_name]])
  }
}

# Make permanant changes
for (location in unique(sub_use$location)) {
  sub_location <- gsub("[ &-]", "_", location)
  execstr <- paste0('sub_use <- mutate(sub_use, is_',
                    sub_location,
                    ' = (sub_use$location == "', location, '") )')
  eval(parse(text = execstr))
}
sub_use <- select(sub_use, !(location))
sub_use$sex <- (sub_use$sex == "Male")
sub_use$measure <- (sub_use$measure == "Deaths")
sub_use$cause <- (sub_use$cause == "Alcohol use disorders")
sub_age <- unique(sub_use$age)
sub_use$age <- as.numeric(as.factor(sub_use$age))
for (col_name in names(sub_use)) {
  if (class(sub_use[[col_name]]) == "logical") {
    sub_use[[col_name]] <- as.numeric(sub_use[[col_name]])
  }
}
...
sub_use_rand <- sub_use[order(runif(15000)),]
train <- slice_head(sub_use_rand, n = 15000 / 2)
train.data <- select(train, !(val))
train.label <- train$val
test <- slice_tail(sub_use_rand, n = 15000 / 2)
test.data <- select(test, !(val))
test.label <- test$val

train <- dplyr::filter(sub_use, measure == 1, sex == 1,
                       age == 2, cause == 1, is_North_America == 1)


# rpart
m_rpart <- rpart(val ~ year, data = train)
m_m5p <- M5P(val ~ year, data = train)
m <- neuralnet(val ~ year, data = train, hidden = 10, threshold = 0.01)
summary(m)
plot(m)
predict_data <- data.frame()
predict_data_slice <- train[1,]
predict_data_slice$val <- 0
for (year in c(2020:2050)) {
  predict_data_slice$year <- year
  predict_data <- rbind(predict_data, predict_data_slice)
}

predict_data$val_neuralnet <- compute(m, predict_data)$net.result[, 1]
predict_data$val_rpart <- predict(m_rpart, predict_data)
predict_data$val_m5p <- predict(m_m5p, predict_data)

train$val_neuralnet <- train$val
train$val_rpart <- train$val
train$val_m5p <- train$val
plot_ds <- rbind(train, predict_data)
ggplot(plot_ds, aes(x = year)) +
        geom_point(aes(y = val), show.legend = TRUE) +
        geom_line(aes(y = val_neuralnet), show.legend = TRUE) +
        geom_line(aes(y = val_rpart), show.legend = TRUE) +
        geom_line(aes(y = val_m5p), show.legend = TRUE) +
        scale_color_viridis(discrete = T)
```
