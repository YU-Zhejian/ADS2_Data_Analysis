...
We conclude that Europe and Central Asia -WB have the highest rate of alcohol-related deaths among man aged 40-44 in 2019.
...
g <- ggplot(sub_use_Opioid, aes(x = year, y = val, color = age)) +
  geom_point(show.legend = TRUE) +
  geom_line(show.legend = TRUE) +
  labs(title = "Deaths of opioid-related disease in the United States",
       x = "Year", y = "Prevalence") +
  facet_grid(. ~ sex) +
  scale_color_viridis(discrete = T)
g
...
From the plot, we can confirm an increase in the prevalence of diseases related to opioid use. Both men and women in 25-29 age group are the most affected.

# Part 2: Ask your own Question

## Question 4: When we plot the "val" according to "year" (taking the prevalence of opioid use disorders as an example), we can find that there's some visible differences among locations. Therefore it leads to our own question: Are there any rules for distinguishing the locations by other attributes?
...
First, we use ggplot function to plot scatter plots of the prevalence value of Opioid use disorders and placet it according to sex and location. 
...
rm(attributeUsage)
```
From the table, we can see that age and cause have a greater impact on region, followed by year and gender.The contribution of measurement indicators is the smallest, which is in line with our common sense.
